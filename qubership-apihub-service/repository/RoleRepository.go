...
type RoleRepository interface {
	...
	GetWorkspacePackageReadAccess(workspaceId string, principal entity.VisibilityPrincipal) ([]entity.PackageReadAccessEntity, error)
...
func (r roleRepositoryImpl) GetWorkspacePackageReadAccess(workspaceId string, principal entity.VisibilityPrincipal) ([]entity.PackageReadAccessEntity, error) {
	if workspaceId == "" {
		return nil, nil
	}
	...
	case entity.VisibilityPrincipalSysadmin:
		...
		WHERE deleted_at IS NULL
		  AND (id = ? OR (id ~>=~ (? || '.') AND id ~<~ (? || '/')))
		`
		_, err = r.cp.GetConnection().Query(&result, query, workspaceId, workspaceId, workspaceId)
	case entity.VisibilityPrincipalApiKey:
		...
		`
		_, err = r.cp.GetConnection().Query(&result, query,
	...
	default:
		...
		WHERE wp.deleted_at IS NULL
		  AND (wp.id = ? OR (wp.id ~>=~ (? || '.') AND wp.id ~<~ (? || '/')))
		`
		_, err = r.cp.GetConnection().Query(&result, query, principal.UserId, workspaceId, workspaceId, workspaceId)
	}
	if err != nil {
		if err == pg.ErrNoRows {
			return nil, nil
