...
func (c comparisonCleanupRepositoryImpl) VacuumComparisonTables(ctx context.Context) error {
	...
	_, err = c.cp.GetConnection().ExecContext(ctx, "VACUUM FULL version_comparison_notification")
	if err != nil {
		errorMsg := fmt.Sprintf("Failed to vacuum 'version_comparison_notification' table: %v", err)
		logger.Warn(ctx, errorMsg)
		vacuumErrors = append(vacuumErrors, errorMsg)
	}
