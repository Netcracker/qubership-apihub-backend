...
type GroupedOperationHistoryPair struct {
	Old []GroupedOperationEntity
	New []GroupedOperationEntity
}

type GroupedOperationHistoryEntity struct {
	tableName struct{} `pg:"grouped_operation_history"`

	GroupId string                      `pg:"group_id,type:varchar"`
	Time    time.Time                   `pg:"time,type:timestamp without time zone"`
	Reason  string                      `pg:"reason,type:varchar"`
	Data    GroupedOperationHistoryPair `pg:"data,type:jsonb"`
}
