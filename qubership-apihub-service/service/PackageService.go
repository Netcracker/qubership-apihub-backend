...
type PackageService interface {
	...
	GetDeletedPackagesList(ctx context.SecurityContext, req view.PackageListReq) (*view.Packages, error)
...
func (p packageServiceImpl) GetPackagesList(ctx context.SecurityContext, searchReq view.PackageListReq) (*view.Packages, error) {
	...
	packages, err := p.publishedRepo.GetReadablePackages(stdctx.Background(), searchReq, scope, ctx.GetUserId())
	if err != nil {
	...
	parents, err := p.parentsByPackage(searchReq, packageIds(packages), false)
	if err != nil {
		return nil, err
	}
	...
	for _, pkg := range packages {
		...
		lastReleaseVersionDetails, err := p.lastReleaseVersionDetails(searchReq, pkg.PackageEntity)
		if err != nil {
			return nil, err
		}
...
func (p packageServiceImpl) GetDeletedPackagesList(ctx context.SecurityContext, searchReq view.PackageListReq) (*view.Packages, error) {
	searchReq = withDefaultKind(searchReq)

	packages, err := p.publishedRepo.GetDeletedPackages(stdctx.Background(), searchReq, ctx.GetUserId())
	if err != nil {
	...
	parents, err := p.parentsByPackage(searchReq, ids, true)
	if err != nil {
		return nil, err
	}
...
func (p packageServiceImpl) parentsByPackage(searchReq view.PackageListReq, ids []string, deleted bool) (map[string][]view.ParentPackageInfo, error) {
	result := make(map[string][]view.ParentPackageInfo, len(ids))
	if !searchReq.ShowParents || len(ids) == 0 {
		return result, nil
	}
	parentsByPackage, err := p.publishedRepo.GetParentsForPackages(ids, deleted)
	if err != nil {
		return nil, err
	}
...
func (p packageServiceImpl) lastReleaseVersionDetails(searchReq view.PackageListReq, pkg entity.PackageEntity) (*view.VersionDetails, error) {
	if !searchReq.LastReleaseVersionDetails {
		return nil, nil
	}
	version := pkg.DefaultReleaseVersion
	if version == "" {
		defaultVersion, err := p.versionService.GetDefaultVersion(pkg.Id)
		if err != nil {
			return nil, err
		}
	...
	return p.versionService.GetVersionDetails(pkg.Id, version)
}
